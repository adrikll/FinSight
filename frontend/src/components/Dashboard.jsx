  ...
  return (
    <div className="space-y-6 pb-6">
      ...
      <div className="flex justify-between items-center bg-slate-900/60 p-6 rounded-lg border border-slate-800/80">
        <div>
          <h2 className="text-2xl font-bold text-white tracking-tight">Panorama Nacional de Crédito</h2>
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <span className="px-4 py-2 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-full text-sm font-semibold block">
              {loading ? 'Sincronizando' : erroApi ? 'Modo Offline' : 'Dados Reais Ativos'}
            </span>
            {metrics?.ultima_atualizacao && (
              <p className="text-xs text-slate-400 mt-1">Dado até {metrics.ultima_atualizacao} · carregado em {metrics.carregado_em}</p>
            )}
          </div>
        </div>
      </div>
      ...
      {erroApi && (
        <div className="p-4 bg-amber-950/40 border border-amber-700/50 text-amber-300 rounded-lg text-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 bg-slate-900/80 p-6 rounded-lg border border-slate-800/80 flex flex-col justify-between shadow-xl">
          <div>
            <h3 className="text-lg font-bold text-white mb-1">Evolução do Crédito</h3>
            <p className="text-sm text-slate-300 mb-4">Carteira total (R$) vs Inadimplência (%)</p>
          </div>
          ...
          <div className="pt-3 border-t border-slate-800 text-sm text-slate-300 text-center font-medium mt-4">
            Série histórica consolidada
          </div>
        </div>

        {/* Gráfico 2: Ranking de Modalidades */}
        <div className="lg:col-span-4 bg-slate-900/80 p-6 rounded-lg border border-slate-800/80 flex flex-col justify-between shadow-xl">
          <div>
            <h3 className="text-lg font-bold text-white mb-1">Ranking de Modalidades</h3>
            <p className="text-sm text-slate-300 mb-4">Volume alocado por produto</p>
          </div>
        ...
        {/* Gráfico 3: Perfil PF vs PJ (Fontes Ampliadas) */}
        <div className="lg:col-span-2 bg-slate-900/80 p-6 rounded-lg border border-slate-800/80 flex flex-col justify-between shadow-xl">
          <div>
            <h3 className="text-lg font-bold text-white mb-1">Perfil PF vs PJ</h3>
            <p className="text-sm text-slate-300 mb-2">Divisão ativa</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center my-auto py-2">
            ...
            {/* CAIXAS DE DETALHES PF E PJ COM FONTES AMPLIADAS */}
            <div className="w-full space-y-2.5 mt-3">
              <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-800 text-xs">
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="text-white font-bold text-sm truncate">Pessoa Física</span>
                </div>
                <div className="flex justify-between items-center text-slate-300 text-xs py-0.5"><span>Cart.:</span> <span className="text-white font-bold">{formatarMoeda(pf?.carteira)}</span></div>
                <div className="flex justify-between items-center text-slate-300 text-xs py-0.5"><span>Inad.:</span> <span className="text-white font-bold">{pf?.taxa_inadimplencia}%</span></div>
              </div>

              <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-800 text-xs">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className="w-3 h-3 rounded-full bg-[#38bdf8] flex-shrink-0"></span>
                  <span className="text-white font-bold text-sm truncate">Pessoa Jurídica</span>
                </div>
                <div className="flex justify-between items-center text-slate-300 text-xs py-0.5"><span>Cart.:</span> <span className="text-white font-bold">{formatarMoeda(pj?.carteira)}</span></div>
                <div className="flex justify-between items-center text-slate-300 text-xs py-0.5"><span>Inad.:</span> <span className="text-white font-bold">{pj?.taxa_inadimplencia}%</span></div>
              </div>
            </div>
          </div>

          <div className="pt-3 border-t border-slate-800 text-sm text-slate-300 text-center font-medium mt-2">
            Segmentação
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 bg-slate-900/80 p-6 rounded-lg border border-slate-800/80 flex flex-col justify-between shadow-xl">
          <div>
            <h3 className="text-lg font-bold text-white mb-1">Ativos Problemáticos Segmentados (PF vs PJ)</h3>
            <p className="text-sm text-slate-300 mb-4">Evolução da taxa de ativos problemáticos (%) por tipo de cliente</p>
          </div>
          <div className="w-full h-[260px]">
            <ResponsiveContainer width="100%" height={260}>
        ...
        <div className="lg:col-span-6 bg-slate-900/80 p-6 rounded-lg border border-slate-800/80 flex flex-col justify-between shadow-xl">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-lg font-bold text-white mb-1">Mapa de Exposição por UF</h3>
              <p className="text-sm text-slate-300">Passe o cursor ou clique para fixar o estado</p>
            </div>
            <div className="flex gap-1.5 bg-slate-950/80 rounded-xl p-1.5 border border-slate-800">
              {METRICAS_MAPA.map(m => (
                <button key={m.key} onClick={() => setMetricaMapa(m.key)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${metricaMapa === m.key ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-300 hover:text-white'}`}>
                  {m.label}
                </button>
              ))}
            </div>
          ...
          {mapaUf.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center my-auto">
              <div className="md:col-span-8 flex items-center justify-center bg-slate-950/50 rounded-lg p-4 border border-slate-800/80 h-[260px]">
                <MapaBrasil 
                  mapaUf={mapaUf}
              ...
              <div className="md:col-span-4 flex flex-col justify-center">
                {ufDetalhe ? (
                  <div className="p-4 bg-slate-950 rounded-lg border border-slate-800 flex flex-col gap-3 text-sm shadow-inner">
                    <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-slate-300">Selecionado</span>
                      </div>
                      <button 
                        onClick={() => setUfSelecionada(null)} 
                        className="text-xs text-slate-200 hover:text-white bg-slate-800 hover:bg-slate-700 px-2 py-0.5 rounded transition-colors"
                        title="Limpar seleção"
                      >
                        ✕ Limpar
                      </button>
                    </div>
                    <div className="flex justify-between text-slate-300"><span>Carteira:</span> <b className="text-white font-semibold">{formatarMoeda(ufDetalhe.carteira)}</b></div>
                    <div className="flex justify-between text-slate-300"><span>Inadimplência:</span> <b className="text-white font-semibold">{ufDetalhe.taxa_inadimplencia}%</b></div>
                    <div className="flex justify-between text-slate-300"><span>Ativos Prob.:</span> <b className="text-white font-semibold">{ufDetalhe.taxa_ativo_problematico}%</b></div>
                  </div>
                ) : (
                  <div className="p-4 bg-slate-950/60 rounded-lg border border-slate-800 flex flex-col items-center justify-center text-center gap-2 text-xs h-[140px]">
                    <p className="text-slate-300 font-medium">Nenhum estado selecionado no momento.</p>
                    <span className="text-slate-300">Clique em um estado do mapa para ver os detalhes.</span>
                  </div>
                )}
              </div>
            </div>
          ) : (
            <p className="text-sm text-slate-300 text-center py-16">Nenhum dado de UF carregado.</p>
          )}
